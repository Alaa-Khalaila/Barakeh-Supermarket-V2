...
router.post("/register", async (req, res) => {
  ...
  if (isExist) return res.json("This email is already registered");
  ...
  if (!user) return res.status(400).json("The user can not be created.");
...
router.post("/login", async (req, res) => {
  ...
  if (user && bcrypt.compareSync(req.body.password, user.passwordHash)) {
    const token = jwt.sign(
      ...
      //TODO: secret should be variable.
      "secret",
      { expiresIn: "1d" }
    );
...
router.put("/:id", async (req, res) => {
  ...
  if (!user) return res.status(400).send("the user cannot be update!");
